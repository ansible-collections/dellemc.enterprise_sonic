...
class Lldp_interfaces(ConfigBase):
    ...
    def _state_merged(self, diff):
        ...
        commands = diff
        requests = []
        requests.extend(self.get_modify_specific_lldp_interfaces_param_requests(commands))
    ...
    def _state_deleted(self, want, have, diff):
        ...
        if not want:
            ...
            requests.extend(self.get_delete_specific_lldp_interfaces_param_requests(commands, have))
    ...
    def get_modify_specific_lldp_interfaces_param_requests(self, commands):
        ...
        if re.search('Eth', name):
            ...
            if 'med_tlv_select' in command and command['med_tlv_select'] is not None:
                if command['med_tlv_select']['power_management'] is not None:
                    med_tlv1 = command['med_tlv_select']['power_management']
                if command['med_tlv_select']['network_policy'] is not None:
    ...
    def get_delete_specific_lldp_interfaces_param_requests(self, commands, config):
        ...
        if re.search('Eth', name):
            if 'enable' in command and command['enable'] is not None:
                ...
                url = self.lldp_intf_config_path['enable'].format(intf_name=name)
                requests.append({'path': url, 'method': PATCH, 'data': payload})
            if command['tlv_set'] is not None:
