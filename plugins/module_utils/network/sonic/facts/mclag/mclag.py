...
from ansible_collections.dellemc.sonic.plugins.module_utils.network.sonic.argspec.mclag.mclag import MclagArgs
        ...
        if conf:
            ...
            if ('vlan-interface' in conf) and (conf['vlan-interface']):
                for each in conf['vlan-interface']:
                    config['unique_ip'].append(each['name'])
            if ('interface' in conf) and (conf['interface']):
                for each in conf['interface']:
                    config['portchannel_members'].append(each['name'])
