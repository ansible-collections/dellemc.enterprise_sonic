...
class VxlansFacts(object):
    ...
    def get_all_vxlans(self):
        ...
        self.fill_tunnel_source_ip(vxlans, vxlan_tunnels)
    ...
    def fill_tunnel_source_ip(self, vxlans, vxlan_tunnels):
    ...
    def fill_vlan_map(self, vxlans, vxlan_vlan_map):
        for each_vlan_map in vxlan_vlan_map:
            ...
            if matched_vtep:
                ...
                vlanmap = matched_vtep.get('vlanmap')
                if vlanmap:
                    vlanmap.append(dict({'vni': vni, 'vlan': vlan}))
                else:
                    matched_vtep['vlanmap'] = [dict({'vni': vni, 'vlan': vlan})]
    ...
    def fill_vrf_map(self, vxlans, vxlan_vrf_list):
        for each_vrf in vxlan_vrf_list:
            ...
            for each_vxlan in vxlans:
                for each_vlan in each_vxlan.get('vlanmap', []):
            ...
            if matched_vtep:
                ...
                vrfmap = matched_vtep.get('vrfmap')
                if vrfmap:
                    vrfmap.append(dict({'vni': vni, 'vrf': vrf}))
                else:
                    matched_vtep['vrfmap'] = [dict({'vni': vni, 'vrf': vrf})]
