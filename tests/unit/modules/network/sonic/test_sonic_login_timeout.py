...
class TestSonicLoginLockoutModule(TestSonicModule):
    ...
    def setUpClass(cls):
        cls.mock_facts_edit_config = patch(
            "ansible_collections.dellemc.enterprise_sonic.plugins.module_utils.network.sonic.facts.login_timeout.login_timeout.edit_config"
        )
        cls.mock_config_edit_config = patch(
            "ansible_collections.dellemc.enterprise_sonic.plugins.module_utils.network.sonic.config.login_timeout.login_timeout.edit_config"
        )
        ...
        cls.fixture_data = cls.load_fixtures('sonic_login_timeout.yaml')

    def setUp(self):
        super(TestSonicLoginTimeoutModule, self).setUp()
    ...
    def tearDown(self):
        super(TestSonicLoginTimeoutModule, self).tearDown()
        self.mock_facts_edit_config.stop()
        self.mock_config_edit_config.stop()
        self.mock_utils_edit_config.stop()

    def test_sonic_login_timeout_merged_01(self):
    ...
    def test_sonic_login_timeout_deleted_01(self):
    ...
    def test_sonic_login_timeout_replaced_01(self):
    ...
    def test_sonic_login_timeout_overridden_01(self):
