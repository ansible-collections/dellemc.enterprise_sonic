...
class TestSonicRoceModule(TestSonicModule):
    module = sonic_roce

    @classmethod
    def setUpClass(cls):
        cls.mock_facts_edit_config = patch(
            "ansible_collections.dellemc.enterprise_sonic.plugins.module_utils.network.sonic.facts.roce.roce.edit_config"
        )
        cls.mock_config_edit_config_reboot = patch(
            "ansible_collections.dellemc.enterprise_sonic.plugins.module_utils.network.sonic.config.roce.roce.edit_config_reboot"
        )
        ...
        cls.fixture_data = cls.load_fixtures('sonic_roce.yaml')

    def setUp(self):
        super(TestSonicRoceModule, self).setUp()
    ...
    def tearDown(self):
        super(TestSonicRoceModule, self).tearDown()
        self.mock_facts_edit_config.stop()
        self.mock_config_edit_config_reboot.stop()
        self.mock_get_interface_naming_mode.stop()

    def test_sonic_roce_merged_01(self):
        set_module_args(self.fixture_data['merged_01']['module_args'])
        self.initialize_facts_get_requests(self.fixture_data['merged_01']['existing_roce_config'])
